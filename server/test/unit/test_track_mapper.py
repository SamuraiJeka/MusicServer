...
def test_track_mapper_create_entity_and_entity_to_dto() -> None:
    duration = timedelta(seconds=180)
    content = b"audio-bytes"

    track: Track = asyncio.run(
        TrackMapper.dto_to_entity(
            owner_id=1,
            title="test-track",
            duration=duration,
            content=content,
    ...
    track.id = 1
    dto = asyncio.run(TrackMapper.entity_to_dto(track))
